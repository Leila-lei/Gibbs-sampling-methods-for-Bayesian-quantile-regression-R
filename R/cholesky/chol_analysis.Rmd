...
library(mcmcse)
rm(list=ls())
setwd('C:\\Users\\juyil\\Dropbox\\Study\\GitHub\\Bayesian-gibbs-sampler\\R\\cholesky')
```
...
for (p in quantiles){
    for (dist_eps in dist_eps_list){
        ...
        for (i in 1:3){
          beta_p[[i]]=unlist(lapply(beta_p_record, function(x){return(x[i,])}))[2001:itr_total]
          # plot(beta_p[[i]], type="l", main=paste("GWS_",p,"_",dist_eps,"_beta_",i), xlab="itr", ylab=paste("t",i))
        }
...
## Very similar to the run time of ordinary matrix inverse calculations.
